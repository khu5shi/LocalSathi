...
const ProfileModal = ({ profile, onClose, onSave, theme }) => {
      ...
      <div className={`rounded-2xl p-8 max-w-2xl w-full ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-4">
            <img src={form.avatar} alt="Profile" className="w-20 h-20 rounded-full" />
          ...
          <input value={form.company} onChange={e => setForm({...form, company: e.target.value})} className="w-full px-4 py-3 border rounded-xl" />
